#!/bin/bash
# Cell-Centered Heat Convergence Test (Item 5)
HEAT_EXEC="${1:-./heat_center_2D}"
if [ ! -f "$HEAT_EXEC" ]; then
  echo "Error: Executable not found: $HEAT_EXEC"; exit 1
fi
echo "=== Cell-Centered Heat Convergence Test ==="
