...
// This function implements the projection step: u^(n+1) = u* - ∇φ
// where φ is the pressure correction stored in element center DOFs
//
// Mathematical basis (Simplified Projection Method):
// 1. Predictor: (u* - u^n)/dt = ν∇²u* + f  (intermediate velocity)
// 2. Poisson:   ∇²φ = ∇·u*                 (pressure correction, φ has units of velocity)
// 3. Corrector: u^(n+1) = u* - ∇φ          (final velocity, ∇·u^(n+1) = 0)
//
// Verification: ∇·u^(n+1) = ∇·u* - ∇²φ = ∇·u* - ∇·u* = 0 ✓
//
// Note: This is the "simplified" form where φ has velocity units (not pressure).
//       The full form would be: u^(n+1) = u* - dt·∇(p/ρ) with ∇²p = ρ/dt·∇·u*
//       But for our purposes, absorbing constants into φ simplifies the implementation.
//
// Discretization:
// - ∂φ/∂x at LEFT edge (ex): (φ[ey][ex] - φ[ey][ex-1]) / hx
// - ∂φ/∂y at DOWN edge (ey): (φ[ey][ex] - φ[ey-1][ex]) / hy
// - Correction: u_new = u_old - ∂φ/∂x
//               v_new = v_old - ∂φ/∂y
//
// Boundary conditions:
...
PetscErrorCode CorrectVelocity(const DM &dm, Vec &u, Vec &uLocal, PetscInt Nx,
                               PetscInt Ny, PetscReal t,
                               FUNC::VELOCITY_EXACT _u_exact, 
                               FUNC::VELOCITY_EXACT _v_exact) {
  PetscFunctionBeginUser;
  const PetscReal hx = 1.0 / Nx;
  const PetscReal hy = 1.0 / Ny;
  ...
  for (PetscInt ey = starty; ey < starty + ny; ++ey) {
    for (PetscInt ex = startx; ex < startx + nx + nEx[0]; ++ex) {
      if (ex == 0 || ex == Nx) {
        ...
        aU[ey][ex][iux] = _u_exact(x, y, t);
        continue;
      }
      
      // ∂φ/∂x ≈ (φ_right - φ_left) / hx
      const PetscScalar p_left = aU[ey][ex - 1][ip];
      const PetscScalar p_right = aU[ey][ex][ip];
      const PetscScalar dpdx = (p_right - p_left) / hx;
      
      // Update velocity: u_new = u_old - ∂φ/∂x
      aU[ey][ex][iux] -= dpdx;
    }
  }

  // Correct v component (on DOWN edges) by subtracting ∂φ/∂y
  ...
  for (PetscInt ey = starty; ey < starty + ny + nEx[1]; ++ey) {
    for (PetscInt ex = startx; ex < startx + nx; ++ex) {
      ...
      // Update velocity: v_new = v_old - ∂φ/∂y
      aU[ey][ex][iuy] -= dpdy;
    }
  }
...
PetscErrorCode ComputeResidualNorm(const DM &dm, const Vec &u,
  ...
  for (PetscInt ey = starty; ey < starty + ny + nEx[1]; ++ey) {
    ...
    for (PetscInt ex = startx; ex < startx + nx; ++ex) {
      ...
      // Down neighbor (ey-1)
      if (ey == 1) {
        ud = _v_exact(cX[ex][icenter], cY[0][iprev], t);
      } else {
        ud = aU[ey - 1][ex][iuy];
      }
      ...
      // Up neighbor (ey+1)
      if (ey == Ny - 1) {
        uu = _v_exact(cX[ex][icenter], cY[Ny][iprev], t);
      } else {
        uu = aU[ey + 1][ex][iuy];
      }
  ...
  for (PetscInt ey = starty; ey < starty + ny; ++ey) {
    for (PetscInt ex = startx; ex < startx + nx + nEx[0]; ++ex) {
      ...
      // Left neighbor (ex-1)
      if (ex == 1) {
        ul = _u_exact(cX[0][iprev], cY[ey][icenter], t);
      } else {
        ul = aU[ey][ex - 1][iux];
      }
      ...
      // Right neighbor (ex+1)
      if (ex == Nx - 1) {
        ur = _u_exact(cX[Nx][iprev], cY[ey][icenter], t);
      } else {
        ur = aU[ey][ex + 1][iux];
      }
...
PetscErrorCode GaussSeidelSweep(const DM &dm, Vec &u, Vec &uLocal,
                                ...
                                PetscReal t, FUNC::VELOCITY_EXACT _u_exact,
                                FUNC::VELOCITY_EXACT _v_exact) {
  PetscFunctionBeginUser;
  const PetscReal hx = 1.0 / Nx;
  const PetscReal hy = 1.0 / Ny;
  ...
  for (int color = 0; color < 2; ++color) {
    ...
    for (PetscInt ey = starty; ey < starty + ny; ++ey) {
      for (PetscInt ex = startx; ex < startx + nx + nEx[0]; ++ex) {
        ...
        // Get neighbor values
        PetscScalar ul = (ex == 1)
                             ? _u_exact(cX[0][iprev], cY[ey][icenter], t)
                             : aU[ey][ex - 1][iux];
        PetscScalar ur =
            (ex == Nx - 1) ? _u_exact(cX[Nx][iprev], cY[ey][icenter], t)
                           : aU[ey][ex + 1][iux];
        PetscScalar ud = (ey == 0)
                             ? 2.0 * _u_exact(cX[ex][iprev], 0.0, t) - uc
                             : aU[ey - 1][ex][iux];
        PetscScalar uu = (ey == Ny - 1)
                             ? 2.0 * _u_exact(cX[ex][iprev], 1.0, t) - uc
                             : aU[ey + 1][ex][iux];
...
PetscErrorCode ComputeL2Error(const DM &dm, const Vec &u, Vec &uLocal,
  ...
  // DOWN edges (including boundary edges) - v component
  for (PetscInt ey = starty; ey < starty + ny + nEx[1]; ++ey) {
    for (PetscInt ex = startx; ex < startx + nx; ++ex) {
      const PetscScalar x = cX[ex][icenter];
      const PetscScalar y = cY[ey][iprev];
      const PetscScalar uExact = _v_exact(x, y, t);
      const PetscScalar diff = aU[ey][ex][iuy] - uExact;
      localError2 += diff * diff;
    }
  }

  // // LEFT edges (including boundary edges) - u component
  // for (PetscInt ey = starty; ey < starty + ny; ++ey) {
  //   for (PetscInt ex = startx; ex < startx + nx + nEx[0]; ++ex) {
  //     const PetscScalar x = cX[ex][iprev];
  //     const PetscScalar y = cY[ey][icenter];
  //     const PetscScalar uExact = _u_exact(x, y, t);
...
int main(int argc, char **argv) {
  ...
  const PetscReal poissonTol = 1e-7;
  ...
  while (t < T_final) {
    ...
    for (its = 1; its <= maxIts; ++its) {
      PetscCall(GaussSeidelSweep(dm, u, uLocal, uOld, uOldLocal, f, fLocal,
                                 Nglob[0], Nglob[1], alpha, dt, t, u_exact, v_exact));
    ...
    // Step 2: Enforce zero mean on Poisson RHS (compatibility condition)
    // Note: We solve ∇²φ = ∇·u*, then correct with u = u* - ∇φ (no dt scaling)
    // PetscCall(PoissonZero(dm, div, divLocal, Nglob[0], Nglob[1]));
    PetscCall(VecScale(div, -1.0));

    
    // Step 3: Solve Poisson equation for pressure correction: ∇²φ = ∇·u*
    ...
    // Step 5: Correct velocity using pressure gradient: u^(n+1) = u* - ∇φ (simplified projection)
    PetscCall(CorrectVelocity(dm, u, uLocal, Nglob[0], Nglob[1], t, u_exact, v_exact));
