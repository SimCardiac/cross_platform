...
public:
	...
	double r = 0.2;
	double p_center[3] = { 0.5, 0.5, 0.5 };//+

	//边界条件-----------------------------------
	int velocity_boundary = 1;
	int pressure_boundary = 1;
	int phi_boundary = 0;
	double hh_phi = 0.0;
	...
	//流体速度系数
	void assembly_Periodic_boundary_condition_NS_coeff_u3D(Mat NS_coeff_u3D);

	void assembly_Periodic_boundary_condition_NS_coeff_v3D(Mat NS_coeff_v3D);

	void assembly_Periodic_boundary_condition_NS_coeff_w3D(Mat NS_coeff_w3D);

	//流体压力系数
	void assembly_Periodic_boundary_condition_NS_coeff_p3D(Mat NS_coeff_p3D);
