...
void wyh_solve_fluid_structure_interaction_system3D200630::set_variable_size(MPI_Comm PETSC_COMM_WORLD)
{
	...
	MatSetSizes(NS_coeff_u3D, PETSC_DECIDE, PETSC_DECIDE, all_mesh_num, all_mesh_num);
	MatSetFromOptions(NS_coeff_u3D);

	MatCreate(PETSC_COMM_WORLD, &NS_coeff_v3D);
	MatSetSizes(NS_coeff_v3D, PETSC_DECIDE, PETSC_DECIDE, all_mesh_num, all_mesh_num);
	MatSetFromOptions(NS_coeff_v3D);

	MatCreate(PETSC_COMM_WORLD, &NS_coeff_w3D);
	MatSetSizes(NS_coeff_w3D, PETSC_DECIDE, PETSC_DECIDE, all_mesh_num, all_mesh_num);
...
void wyh_solve_fluid_structure_interaction_system3D200630::prepare_assembly_matrix_and_vector()
{
	...
	//方程组系数
	assembly_Periodic_boundary_condition_NS_coeff_u3D(NS_coeff_u3D);
	assembly_Periodic_boundary_condition_NS_coeff_v3D(NS_coeff_v3D);
	assembly_Periodic_boundary_condition_NS_coeff_w3D(NS_coeff_w3D);
	assembly_Periodic_boundary_condition_NS_coeff_p3D(NS_coeff_p3D);
...
void wyh_solve_fluid_structure_interaction_system3D200630::solve_fluid_structure_interaction_system3D()
{
	...
	//求解NS方程
	KSPSolve(KSP_NSeq_u, u_NS_right_term, velocity_u_star);
	KSPSolve(KSP_NSeq_v, v_NS_right_term, velocity_v_star);
	KSPSolve(KSP_NSeq_w, w_NS_right_term, velocity_w_star);
...
void wyh_solve_fluid_structure_interaction_system3D200630::get_initial_condition()
{
	...
	VecGetOwnershipRange(velocity_u, &istart, &iend);
	for (PetscInt Ii = istart; Ii < iend; Ii++) {
		...
		double node_x_at_u_temp = x_L + j*hx;
		double node_y_at_u_temp = y_L + hy / 2.0 + i*hy;
		double node_z_at_u_temp = z_L + hz / 2.0 + k*hz;

		double node_x_at_w_temp = x_L + hx / 2.0 + j*hx;
		double node_y_at_w_temp = y_L + hy / 2.0 + i*hy;
		double node_z_at_w_temp = z_L + k*hz;

		double u_temp;
		double w_temp;

		u_temp = 5e-2*sin(kx_temp*node_x_at_u_temp) * cos(ky_temp*node_z_at_u_temp) * 2 * pi;
		w_temp = -5e-2*cos(kx_temp*node_x_at_w_temp) * sin(ky_temp*node_z_at_w_temp) * 2 * pi;

		VecSetValues(velocity_u, 1, &Ii, &u_temp, INSERT_VALUES);
		VecSetValues(velocity_w, 1, &Ii, &w_temp, INSERT_VALUES);
	}
	VecAssemblyBegin(velocity_u);
	VecAssemblyEnd(velocity_u);
	VecAssemblyBegin(velocity_w);
	VecAssemblyEnd(velocity_w);
...
//流体速度系数------------------------------------
void wyh_solve_fluid_structure_interaction_system3D200630::assembly_Periodic_boundary_condition_NS_coeff_u3D(Mat NS_coeff_u3D)
{
...
void wyh_solve_fluid_structure_interaction_system3D200630::assembly_Periodic_boundary_condition_NS_coeff_v3D(Mat NS_coeff_v3D)
{
	int m_num_at_coeff = m_num;
	int n_num_at_coeff = n_num;
	int o_num_at_coeff = o_num;
...
void wyh_solve_fluid_structure_interaction_system3D200630::assembly_Periodic_boundary_condition_NS_coeff_w3D(Mat NS_coeff_w3D)
{
	int m_num_at_coeff = m_num;
	int n_num_at_coeff = n_num;
	int o_num_at_coeff = o_num;
...
//流体压力系数------------------------------------
void wyh_solve_fluid_structure_interaction_system3D200630::assembly_Periodic_boundary_condition_NS_coeff_p3D(Mat NS_coeff_p3D)
{
	int m_num_at_coeff = m_num;
	int n_num_at_coeff = n_num;
	int o_num_at_coeff = o_num;
...
void wyh_solve_fluid_structure_interaction_system3D200630::modified_phi_based_on_intermediate_step()
{
	...
	double disturbance = 1.e-3;
	double d_tau = 0.001*dt;
	double hh = 1e-8;
