  ...
  for (int color = 0; color < 2; ++color) {
    ...
    for (PetscInt ey = starty; ey < starty + ny + nEx[1]; ++ey) {
      ...
      for (PetscInt ex = startx; ex < startx + nx; ++ex) {
        ...
        aU[ey][ex][iuy] = (rhs + coef * (ix2 * (ul + ur) + iy2 * (ud + uu))) / diag;
      }
    }
    ...
    for (PetscInt ey = starty; ey < starty + ny; ++ey) {
      for (PetscInt ex = startx; ex < startx + nx + nEx[0]; ++ex) {
        ...
        aU[ey][ex][iux] = (rhs + coef * (ix2 * (ul + ur) + iy2 * (ud + uu))) / diag;
      }
    }
...
int main(int argc, char **argv) {
  ...
  PetscCall(PetscOptionsGetBool(NULL, NULL, "-check_error", &compute_error, NULL));
  ...
  
  {
    ...
    if (compute_error || convergence_test) {
      PetscReal l2Error;
      PetscCall(ComputeL2Error_Heat(dm, u, uLocal, Nglob[0], Nglob[1], t, alpha, &l2Error));
      if (rank == 0) {
