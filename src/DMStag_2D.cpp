...
// 目的: 更加精确的 不可压缩 Navier-Stokes 方程求解程序.
// 关键在于使用二阶格式处理对流项
// 标准的展示结果，在一条线上的函数值:
// [0.5, 0.0, 0.0] -> [0.5, 1.0, 0.0], 1000 个点
// 下面是数据。
...
int main(int argc, char **argv)
{
  ...
  PetscCall(PetscInitialize(&argc, &argv, NULL, "DMStag 2D example solving a Stokes-like system"));
  ...
  /* Define uniform coordinates as a product of 1D arrays [0,1]x[0,1]x[0,0] */
...
static PetscErrorCode CreateReferenceSolution(DM dmSol, Vec *pSolRef)
{
  ...
  PetscCall(DMStagGetLocationSlot(dmSol, DOWN, 0, &iuy)); /* iuy 表示单元下边界的第 0 个分量 */
