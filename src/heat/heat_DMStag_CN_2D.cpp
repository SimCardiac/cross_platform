...
// ============================================================================
// Compute residual norm for Crank-Nicolson
// Crank-Nicolson: (I - 0.5*dt*alpha*L)*u^{n+1} = (I + 0.5*dt*alpha*L)*u^n + dt*f^{n+1/2}
// ============================================================================
PetscErrorCode ComputeResidualNorm(const DM &dm, 
                                   const Vec &u, const Vec &uLocal, 
                                   const Vec &uOld, const Vec &uOldLocal, 
                                   const Vec &fMid, const Vec &fMidLocal,
  ...
  PetscCall(DMGlobalToLocalBegin(dm, fMid, INSERT_VALUES, fMidLocal));
  PetscCall(DMGlobalToLocalEnd(dm, fMid, INSERT_VALUES, fMidLocal));
  PetscCall(DMStagVecGetArray(dm, uLocal, &aU));
  PetscCall(DMStagVecGetArray(dm, uOldLocal, &aUold));
  PetscCall(DMStagVecGetArray(dm, fMidLocal, &aF));
  PetscCall(DMStagGetCorners(dm, &startx, &starty, NULL, &nx, &ny, NULL, &nEx[0], &nEx[1], NULL));
  ...
  for (PetscInt ey = starty; ey < starty + ny; ++ey) {
    for (PetscInt ex = startx; ex < startx + nx; ++ex) {
      ...
      // Get neighbor values for u^{n+1} (reflecting Dirichlet at boundaries)
      ...
      // Get neighbor values for u^n
      const PetscScalar ul_old = DMStag_GetLeft(aUold, ex, ey, icenter, Nx);
      const PetscScalar ur_old = DMStag_GetRight(aUold, ex, ey, icenter, Nx);
      const PetscScalar ud_old = DMStag_GetDown(aUold, ex, ey, icenter, Ny);
      const PetscScalar uu_old = DMStag_GetUp(aUold, ex, ey, icenter, Ny);

      // LHS: (I - 0.5*dt*alpha*L)*u^{n+1}
      const PetscScalar lhs = diag * uc - coef * (ix2 * (ul + ur) + iy2 * (ud + uu));
      
      // RHS: (I + 0.5*dt*alpha*L)*u^n + dt*f^{n+1/2}
      const PetscScalar rhs_implicit = (1.0 - 2.0 * coef * (ix2 + iy2)) * uc_old 
                                     + coef * (ix2 * (ul_old + ur_old) + iy2 * (ud_old + uu_old));
      const PetscScalar rhs = rhs_implicit + dt * aF[ey][ex][icenter];
  ...
  PetscCall(DMStagVecRestoreArray(dm, fMidLocal, &aF));
  PetscReal outNorm;
...
// ============================================================================
// Perform one red-black Gauss-Seidel sweep for Crank-Nicolson
// ============================================================================
  ...
  for (int color = 0; color < 2; ++color) {
    ...
    for (PetscInt ey = starty; ey < starty + ny; ++ey) {
      for (PetscInt ex = startx; ex < startx + nx; ++ex) {
        ...
        // Solve for u^{n+1}: diag * u^{n+1} - 0.5*dt*alpha*L*u^{n+1} = rhs
...
int main(int argc, char **argv) {
  PetscFunctionBeginUser;
  PetscCall(PetscInitialize(&argc, &argv, NULL, "Heat equation solver using Crank-Nicolson with DMStag"));

  DM dm;
  Vec u, uLocal, uOld, uOldLocal, fMid, fMidLocal;
  ...
  PetscCall(DMCreateGlobalVector(dm, &fMid));
  PetscCall(DMGetLocalVector(dm, &uLocal));
  PetscCall(DMGetLocalVector(dm, &uOldLocal));
  PetscCall(DMGetLocalVector(dm, &fMidLocal));
  ...
  if (rank == 0) {
    std::cout << "=== Heat Equation Solver (Crank-Nicolson) ===" << std::endl;
    ...
    std::cout << "Time discretization: Crank-Nicolson (2nd order)" << std::endl;
  }
  ...
  while (t < T_final) {
    ...
    // Setup RHS at mid-point time t^{n+1/2} = t^n + dt/2
    // For f=0, this doesn't matter, but kept for generality
    PetscCall(SetupRHS(dm, fMid, fMidLocal, t - 0.5*dt, rhs_f));
    
    // Solve (I - 0.5*dt*alpha*L)*u^{n+1} = (I + 0.5*dt*alpha*L)*u^n + dt*f using Gauss-Seidel
    PetscReal resNorm = 0.0;
    PetscInt its = 0;
    
    for (its = 1; its <= maxIts; ++its) {
      PetscCall(GaussSeidelSweep(dm, u, uLocal, uOld, uOldLocal, fMid, fMidLocal,
                                 Nglob[0], Nglob[1], alpha, dt));
      PetscCall(ComputeResidualNorm(dm, u, uLocal, uOld, uOldLocal, fMid, fMidLocal,
  ...
  PetscCall(DMRestoreLocalVector(dm, &fMidLocal));
  PetscCall(VecDestroy(&u));
  PetscCall(VecDestroy(&uOld));
  PetscCall(VecDestroy(&fMid));
