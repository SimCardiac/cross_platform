...
#include <cstring>
#include <cmath>
#include "poisson/poisson_mms.hpp"
#include "poisson/poisson_metrics.hpp"
#include "poisson/poisson_solver.hpp"
...
int main(int argc, char** argv) {
    ...
    // T008-T011: PETSc/KSP setup, nullspace, JSON output (to be implemented)
    // T009: run_poisson_case wrapper (to be implemented)
    ...
    for (const auto& pair_id : config.pairs) {
        ...
        try {
            const PoissonMMS::MMSPair* pair = PoissonMMS::get_pair(pair_id);
            ...
            ierr = run_poisson_case(&dm, &u, &A, &f,
                                     config.nx, config.ny,
            ...
            ierr = PoissonMetrics::compute_l2_error(dm, u, pair->u_exact, &l2_error); CHKERRQ(ierr);
            ierr = PoissonMetrics::compute_linf_error(dm, u, pair->u_exact, &linf_error); CHKERRQ(ierr);
            ierr = PoissonMetrics::compute_residual_norm(A, u, f, &residual_norm); CHKERRQ(ierr);
